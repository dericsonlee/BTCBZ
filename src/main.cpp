...
uint256 hashGenesisBlock("0xe53528389656560424a8f241e2bd8e644d44965266f9fac9929bba49e254aa22");
...
const string strMessageMagic = "BTCBITZCoin Signed Message:\n";
...
{
    // BTCBITZCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BTCBITZCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "bz genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0xf2c2c88c2c4b25962dd726a69f151190afbe1c6f7f708d5f7f20a590a553999c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd0, 0xc1, 0xae, 0xe3 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BTCBITZCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BTCBITZCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BTCBITZCoinMiner(CWallet *pwallet)
{
    printf("BTCBITZCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BTCBITZCoin-miner");
    ...
    try { loop {
        ...
        printf("Running BTCBITZCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BTCBITZCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BTCBITZCoinMiner, pwallet));
}
