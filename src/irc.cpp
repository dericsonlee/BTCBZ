...
// Copyright (c) 2011-2012 BTCBITZCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("BTCBITZCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BTCBITZCoinTEST0\r");
            Send(hSocket, "WHO #BTCBITZCoinTEST0\r");
        } else {
            // randomly join #BTCBITZCoin00-#BTCBITZCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #BTCBITZCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BTCBITZCoin%02d\r", channel_number).c_str());
        }
